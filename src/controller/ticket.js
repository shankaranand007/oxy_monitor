        ...
        async.parallel({
            ...
            totalReturn: function (callback) {
                returnModel.find({})
                    .exec((err, data) => {
                        if (err) callback(err, data)
                        let ss =  (data.length) ? data.reduce((a, b) => ({"Number_of_cylinder": a.available_oxygen_cylinder + parseInt(b.available_oxygen_cylinder),"available_oxy_meters": a.available_oxy_meters + (b.available_oxy_meters)?parseInt(b.available_oxy_meters):0})):{};
                        callback(null, ss)
                    })
            },
